...
VOID VerifyMainFest(HMODULE hMod, PROCRES *pprocres, DWORD dwSize);
...
BOOL CALLBACK ProcessNamesInMod(HMODULE hMod, LPCTSTR lpszType, LPTSTR lpszName, LONG_PTR lParam)
{
    ...
    procres.bHasMultiLang = lpszType == RT_DIALOG || lpszType == RT_MENU || lpszType == RT_STRING|| lpszType == RT_MANIFEST;
    ...
    if (procres.bHasMultiLang)
    {
        ...
        else if (lpszType == RT_MANIFEST)
        {
			DWORD dwSize = ::SizeofResource((HMODULE)hMod, FindResourceEx(hMod, lpszType, lpszName, rglcidInUse[0]));
			VerifyMainFest(hMod, &procres,dwSize);
        }
...
VOID VerifyMainFest(HMODULE hMod, PROCRES *pprocres, DWORD dwSize)
{
	...
	for (int ilocale = 0; ilocale < iLocaleMac; ilocale++)
	{
		BYTE* lpResrc = (BYTE*)LockResource(pprocres->rghglobSeen[ilocale]);
		if (lpResrc == NULL)
			return;
		...
		std::string sResult = Utf8ToGB2312(pBuffer);
		delete[] pBuffer;
